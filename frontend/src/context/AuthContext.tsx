...
export function AuthProvider({ children }: { children: React.ReactNode }): ReactElement<any, any> {
  ...
  const login: (email: string, password: string) => Promise<{ok:boolean, mode:string}> = async (email: string, password: string): Promise<{ok:boolean; mode:string}> => {
    setLoading(true);
    try {
  ...
  const register: ({name, email, password}: RegisterForm) => Promise<{
    ok: boolean;
    mode: string;
  }> = async ({ name, email, password }: RegisterForm): Promise<{ok: true, mode: string}> => {
    setLoading(true);
    try {
  ...
  const value: AuthContextValue = useMemo(
    (): AuthContextValue => ({ user, setUser, loading, login, loginWithToken, register, logout }),
    [user, loading],
  );
